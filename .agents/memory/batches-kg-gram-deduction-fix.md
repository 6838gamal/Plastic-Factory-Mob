...
**Do NOT auto-resolve** duplicate groups where *all* siblings are zero-balance (~28 groups: PVC صيني, DOP زيت, بيكربونات ابيض/اصفر, لاصق 6022/6026/6031/703/8022/8026/8031, شمع, كالسيوم, سيتريك أسيد, سكراب أزرق/أسود ناعم, several صبغة color variants, etc.) — there's no data signal for which UUID is "correct", so picking one risks masking the real one when the user later enters stock.

**Follow-up per user request:** instead of merging/deleting these ambiguous zero-balance duplicates, renamed each in place to `Original_Name_With_Underscores_N` (e.g. `لاصق_6022_1` / `لاصق_6022_2`) so the batch-entry picker shows visually distinct labels instead of identical text — confirmed via `GET /api/materials` that zero exact-duplicate names remain. User still needs to decide which numbered sibling to actually stock/merge going forward.
