...
async def upsert_material(body: MaterialUpsert):
    ...
    if body.id:
        row = await pool.fetchrow(
            """UPDATE raw_materials SET name=$1, category=$2, unit=$3,
               min_stock=$4, is_active=$5, notes=$6, updated_at=NOW()
        ...
        row = await pool.fetchrow(
            ...
            body.name, body.category, body.unit, body.min_stock, body.is_active, body.notes,
        )
    return dict(row)
